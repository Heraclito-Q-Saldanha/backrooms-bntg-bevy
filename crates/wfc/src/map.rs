...
impl<T: Clone> Map2D<T> {
	pub fn new(size: math::I64Vec2, default: T) -> Self {
		let data = vec![default; (size.x * size.y) as usize];
		Self { data, size }
	}
	pub fn from_vec(data: Vec<T>, size: math::I64Vec2) -> Self {
		let len = (size.x * size.y) as usize;
		assert!(data.len() == len);
		Self { data, size }
	}
	#[inline(always)]
	pub fn index(&self, position: math::I64Vec2) -> usize {
		((self.size.x * position.y) + position.x) as usize
	}
	#[inline(always)]
	pub fn size(&self) -> math::I64Vec2 {
		self.size
	}
	#[inline(always)]
	pub fn position(&self, index: usize) -> math::I64Vec2 {
	...
	#[inline(always)]
	pub fn is_in_range(&self, position: math::I64Vec2) -> bool {
		position.x >= 0 && position.y >= 0 && position.x < self.size.x && position.y < self.size.y
	}
	#[inline(always)]
	pub fn set_tile(&mut self, position: math::I64Vec2, tile: T) {
	...
	#[inline(always)]
	pub fn get_tile(&self, position: math::I64Vec2) -> Option<T> {
		if !self.is_in_range(position) {
			return None;
		}
		let index = self.index(position);
...
impl<T: Tile + Clone + 'static + PartialEq> Map2D<T> {
	pub fn generate(size: math::I64Vec2, seed: u64) -> Result<Map2D<T>, ()> {
		...
		loop {
			...
			while let Some(position) = queue.pop_front() {
				...
				let right = position + math::i64vec2(1, 0);
				let left = position + math::i64vec2(-1, 0);
				let up = position + math::i64vec2(0, 1);
				let down = position + math::i64vec2(0, -1);
